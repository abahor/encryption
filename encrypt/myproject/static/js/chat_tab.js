var listening = io.connect('https://127.0.0.1:5000/start_chat') // put the name and  the route of the server
...
listening.on('receive',function (msg) {
  var text = decrypt_message(msg['text'])
  let temp = template_receive.content.cloneNode(true)
  temp.querySelector('.subject').innerHTML = text
  temp.querySelector('.current_date').innerHTML = date_now() // i can do this on the server but let's do it here any way
...
$(document).ready(function(){
  $('#btn-chat').on('click',function () {
    ...
    listening.emit('send_message',{'message': message, 'token':token,'hisid':his_id})
